    ...
    const isInline = !className;
     ...
     <code className="px-1 py-0.5 bg-gray-100 rounded text-sm" {...props}>
      {children}
        ...
        blackquote({children}){
            return <blackquote className='border-l-4 border-gray-200 pl-4 italic my-4'>{children}</blackquote>
        },
        h1({ children }) {
...
function CodeBlock({ code, language }) {
  ...
  return <div className="relative rounded-lg overflow-hidden bg-gray-50 border border-gray-200">
  <div className="flex items-center justify-between px-4 py-2 bg-gray-100 border-b border-gray-200">
    ...
    >
      ...
      {copied && (
        <span className="absolute -top-8 right-0 bg-black text-white text-xs rounded-md px-2 py-1 opacity-80 group-hover:opacity-100 trasition">
          Copied!
        </span>
      )}
    </button>
